...
if (isset($_GET['feeds']) && !empty($_GET['feeds']))
{
	...
	foreach($_GET['feeds'] as $embedded_feed)
	{
		...
		$checksum = checksum(normalize_url($embedded_feed['href']));
...
foreach($feed_meta['feeds'] as $j => $alt_feed)
{
	...
	{
		$display_title = normalize_url($alt_feed['href']);
	}
	else
	{
		$display_title .= ' ('.normalize_url($alt_feed['href']).')';
	}
	$menu_options[] = "{value:'{$alt_feed['href']}',text:'{$display_title}'}";
}	
...
<?php if (!$feed['is_subscribed']):?>
			<td class="proto">
					...
					<span class="i"><input class="select-all" type="checkbox" id="feed-<?php e($i); ?>" name="feeds[<?php e($i); ?>][url]" value="<?php e($feed['href']); ?>"<?php e($total_feeds == 1 ? ' checked="checked"' : '');?> /></span>
					Add <strong title="<?php e($alt_feed['href']);?>"><?php e($title);?></strong>
				</label>
			</td>
			<td>
...
<?php else:?>
			<td colspan="2">
				...
				Already subscribed to <strong title="<?php e($alt_feed['href']);?>"><?php e($title); ?></strong>
			</td>
<?php endif;?>
		</tr>
<?php $i++; ?>
